  ...
  public:
    ...
    void transition(FlightState next)
    {
      if (node_ != nullptr) {
        ...
        publish_flight_state(state_strs_[next]);
      }
    ...
    void Load(gazebo::physics::ModelPtr model, sdf::ElementPtr sdf)
    {
      ...
      flight_state_pub_ = node_->create_publisher<std_msgs::msg::String>("flight_state", 1); // 创建新的发布器
      ...
      publish_flight_state("landed"); // 发布初始飞行状态
...
#pragma clang diagnostic pop
    ...
    void publish_flight_state(const char* state)
    {
      RCLCPP_INFO(node_->get_logger(), "Publishing flight state: %s", state);
