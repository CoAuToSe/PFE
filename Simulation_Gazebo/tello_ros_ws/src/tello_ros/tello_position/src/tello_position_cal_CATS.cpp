...
public:
    TelloPosition(const rclcpp::NodeOptions & options) :
        Node("tello_position", options), x_(0.0), y_(0.0), z_(0.0), vx_(0.0), vy_(0.0), vz_(0.0), last_time_(this->now())
    {
        subscription_ = this->create_subscription<tello_msgs::msg::FlightData>(
            "flight_data", // need to remove the "/" but for that the publisher needs to be changed // or maybe not lol
            10, 
            std::bind(
...
private:
    void flight_data_callback(const tello_msgs::msg::FlightData::SharedPtr msg)
    {
        ...
        if (msg->tof == 6553) {
            height_tof = z_;  // 使用当前高度估计值 => "Utiliser la valeur actuelle estimée de la hauteur"
        }
        ...
        RCLCPP_INFO(this->get_logger(), "Current position: x=%.2f y=%.2f z=%.2f", x_, y_, z_);
        RCLCPP_INFO(this->get_logger(), "Current speed: vx=%.2f vy=%.2f vz=%.2f", vx_, vy_, vz_);
