   ...
   public:
    TelloPosition(const rclcpp::NodeOptions& options)
        ...
        RCLCPP_INFO(this->get_logger(),
                    "Initial position: x=%.2f y=%.2f z=%.2f", x_, y_, z_);
        ...
        // std::cout << "Fully resolved topic: " <<
        // node_->resolve_topic_name(topic_name_).c_str() << std::endl;
        std::cout << "-----------------------------------------" << std::endl;
        std::cout << std::endl;
    }
   ...
   private:
    void flight_data_callback(const tello_msgs::msg::FlightData::SharedPtr msg) {
        ...
        double vx_natural =
            ...
            msg->vgz * (std::sin(yaw) * std::sin(roll) +
                        std::cos(yaw) * std::sin(pitch) * std::cos(roll));

        double vy_natural =
            msg->vgx * (std::sin(yaw) * std::cos(pitch)) +
            msg->vgy * (std::cos(yaw) * std::cos(roll) +
                        std::sin(yaw) * std::sin(pitch) * std::sin(roll)) +
            msg->vgz * (-std::cos(yaw) * std::sin(roll) +
